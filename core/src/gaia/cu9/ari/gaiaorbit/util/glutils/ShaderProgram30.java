 ...
 * {@link ShaderProgram30#begin()} method must be used which effectively binds
 * the program.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * A ShaderProgram can be unbound with a call to {@link ShaderProgram30#end()}
 * </p>
 * 
 * <p>
 * A ShaderProgram must be disposed via a call to
 * {@link ShaderProgram30#dispose()} when it is no longer needed
 * </p>
 * 
 * <p>
 ...
 * @author mzechner
 */
public class ShaderProgram30 implements Disposable {
    ...
    private final static ObjectMap<Application, Array<ShaderProgram30>> shaders = new ObjectMap<Application, Array<ShaderProgram30>>();
    ...
    public ShaderProgram30(String vertexShader, String fragmentShader) {
    ...
    public ShaderProgram30(FileHandle vertexShader, FileHandle fragmentShader) {
    ...
    private void compileShaders(String vertexShader, String fragmentShader) {
        vertexShaderHandle = loadShader(GL30.GL_VERTEX_SHADER, vertexShader);
        fragmentShaderHandle = loadShader(GL30.GL_FRAGMENT_SHADER, fragmentShader);
    ...
    private int loadShader(int type, String source) {
        ...
        gl.glGetShaderiv(shader, GL30.GL_COMPILE_STATUS, intbuf);
    ...
    private int linkProgram(int program) {
        ...
        gl.glGetProgramiv(program, GL30.GL_LINK_STATUS, intbuf);
        int linked = intbuf.get(0);
        if (linked == 0) {
            // Gdx.gl30.glGetProgramiv(program, GL30.GL_INFO_LOG_LENGTH, intbuf);
            // int infoLogLength = intbuf.get(0);
            // if (infoLogLength > 1) {
            log = Gdx.gl30.glGetProgramInfoLog(program);
            // }
            return -1;
        }
    ...
    /**
     * Sets the uniform with the given name. The {@link ShaderProgram30} must be
    ...
    /**
     * Sets the uniform matrix with the given name. The {@link ShaderProgram30}
     ...
     * {@link ShaderProgram30} must be bound for this to work.
     * 
     * @param name
     *            the name of the uniform
     * @param buffer
     ...
     * @param type
     *            the type, must be one of GL30.GL_BYTE, GL30.GL_UNSIGNED_BYTE,
     *            GL30.GL_SHORT, GL30.GL_UNSIGNED_SHORT,GL30.GL_FIXED, or
     *            GL30.GL_FLOAT. GL_FIXED will not work on the desktop
     * @param normalize
     ...
     * are done with this shader you have to call {@link ShaderProgram30#end()}.
     */
    public void begin() {
        GL30 gl = Gdx.gl30;
        checkManaged();
    ...
    private void addManagedShader(Application app, ShaderProgram30 shaderProgram) {
        Array<ShaderProgram30> managedResources = shaders.get(app);
        if (managedResources == null)
            managedResources = new Array<ShaderProgram30>();
    ...
    public static void invalidateAllShaderPrograms(Application app) {
        ...
        Array<ShaderProgram30> shaderArray = shaders.get(app);
        if (shaderArray == null)
            return;
    ...
    private void fetchUniforms() {
        params.clear();
        Gdx.gl30.glGetProgramiv(program, GL30.GL_ACTIVE_UNIFORMS, params);
        ...
        for (int i = 0; i < numUniforms; i++) {
            ...
            params.put(0, 1);
            type.clear();
            String name = Gdx.gl30.glGetActiveUniform(program, i, params, type);
            int location = Gdx.gl30.glGetUniformLocation(program, name);
            uniforms.put(name, location);
    ...
    private void fetchAttributes() {
        params.clear();
        Gdx.gl30.glGetProgramiv(program, GL30.GL_ACTIVE_ATTRIBUTES, params);
        ...
        for (int i = 0; i < numAttributes; i++) {
            ...
            params.put(0, 1);
            type.clear();
            String name = Gdx.gl30.glGetActiveAttrib(program, i, params, type);
            int location = Gdx.gl30.glGetAttribLocation(program, name);
            attributes.put(name, location);
     ...
     * @param name
     *            the name of the attribute
     * @return the type of the attribute, one of {@link GL30#GL_FLOAT},
     *         {@link GL30#GL_FLOAT_VEC2} etc.
     */
     ...
     * @param name
     *            the name of the uniform
     * @return the type of the uniform, one of {@link GL30#GL_FLOAT},
     *         {@link GL30#GL_FLOAT_VEC2} etc.
     */
