...
package gaia.cu9.ari.gaiaorbit.util.glutils;
...
/**
 * <p>
 * A Mesh30 holds vertices composed of attributes specified by a
 ...
 * application or receives an incoming call. A managed Mesh30 will be reloaded
 * automagically so you don't have to do this manually.
 * </p>
 * 
 * <p>
 * A Mesh30 consists of vertices and optionally indices which specify which
 ...
 * attribute has an alias which is used when rendering a Mesh30 in OpenGL ES
 * 2.0. The alias is used to bind a specific vertex attribute to a shader
 * attribute. The shader source and the alias of the attribute must match
 * exactly for this to work.
 * </p>
 * 
 * @author mzechner, Dave Clayton <contact@redskyforge.com>, Xoppa
 */
public class Mesh30 implements Disposable {
    public enum VertexDataType {
        VertexArray, VertexBufferObject, VertexBufferObjectSubData, VertexBufferObjectWithVAO
    }

    /** list of all meshes **/
    static final Map<Application, Array<Mesh30>> meshes = new HashMap<Application, Array<Mesh30>>();

    final VertexData30 vertices;
    final IndexData indices;
    boolean autoBind = true;
    final boolean isVertexArray;

    protected Mesh30(VertexData30 vertices, IndexData indices, boolean isVertexArray) {
    ...
    /**
     * Creates a new Mesh30 with the given attributes.
     * 
     * @param isStatic
     ...
     *            coordinate
     */
    public Mesh30(boolean isStatic, int maxVertices, int maxIndices, VertexAttribute... attributes) {
     ...
     *            coordinate
     */
    public Mesh30(boolean isStatic, int maxVertices, int maxIndices, VertexAttributes attributes) {
    ...
    /**
     * by jw: Creates a new Mesh30 with the given attributes. Adds extra
     ...
     **/
    public Mesh30(boolean staticVertices, boolean staticIndices, int maxVertices, int maxIndices, VertexAttributes attributes) {
    ...
    private VertexData30 makeVertexBuffer(boolean isStatic, int maxVertices, VertexAttributes vertexAttributes) {
        if (Gdx.gl30 != null) {
            return new VertexBufferObjectWithVAO30(isStatic, maxVertices, vertexAttributes);
        } else {
            return new VertexBufferObject30(isStatic, maxVertices, vertexAttributes);
        }
    }

    /**
     * Creates a new Mesh30 with the given attributes. This is an expert method
     ...
     *            coordinate
     */
    public Mesh30(VertexDataType type, boolean isStatic, int maxVertices, int maxIndices, VertexAttribute... attributes) {
        switch (type) {
        case VertexBufferObject:
            vertices = new VertexBufferObject30(isStatic, maxVertices, attributes);
            indices = new IndexBufferObject(isStatic, maxIndices);
            isVertexArray = false;
            break;
        case VertexBufferObjectSubData:
            vertices = new VertexBufferObjectSubData30(isStatic, maxVertices, attributes);
            indices = new IndexBufferObjectSubData(isStatic, maxIndices);
            isVertexArray = false;
            break;
        case VertexBufferObjectWithVAO:
            vertices = new VertexBufferObjectWithVAO30(isStatic, maxVertices, attributes);
            indices = new IndexBufferObjectSubData(isStatic, maxIndices);
            isVertexArray = false;
            break;
        case VertexArray:
        default:
            vertices = new VertexArray30(maxVertices, attributes);
    ...
    /**
     * Sets the vertices of this Mesh30. The attributes are assumed to be given
     * in float format.
     * 
     * @param vertices
     *            the vertices.
     ...
     */
    public Mesh30 setVertices(float[] vertices) {
     ...
     * @return the mesh for invocation chaining.
     */
    public Mesh30 setVertices(float[] vertices, int offset, int count) {
     ...
     */
    public Mesh30 updateVertices(int targetOffset, float[] source) {
     ...
     */
    public Mesh30 updateVertices(int targetOffset, float[] source, int sourceOffset, int count) {
        this.vertices.updateVertices(targetOffset, source, sourceOffset, count);
        return this;
    }

    /**
     * Copies the vertices from the Mesh30 to the float array. The float array
     * must be large enough to hold all the Mesh30's vertices.
     * 
     * @param vertices
    ...
    /**
     * Copies the the remaining vertices from the Mesh30 to the float array. The
    ...
    /**
     * Copies the specified vertices from the Mesh30 to the float array. The
     * float array must be large enough to hold count vertices.
     * 
     * @param srcOffset
     *            the offset (in number of floats) of the vertices in the mesh
     *            to copy
    ...
    /**
     * Copies the specified vertices from the Mesh30 to the float array. The
     * float array must be large enough to hold destOffset+count vertices.
     * 
     * @param srcOffset
     *            the offset (in number of floats) of the vertices in the mesh
     *            to copy
    ...
    /**
     * Sets the indices of this Mesh30
     * 
     * @param indices
     *            the indices
     * @return the mesh for invocation chaining.
     */
    public Mesh30 setIndices(short[] indices) {
     ...
     * @return the mesh for invocation chaining.
     */
    public Mesh30 setIndices(short[] indices, int offset, int count) {
    ...
    /**
     * Copies the indices from the Mesh30 to the short array. The short array
     * must be large enough to hold all the Mesh30's indices.
     * 
     * @param indices
    ...
    /**
     * Copies the indices from the Mesh30 to the short array. The short array
     * must be large enough to hold destOffset + all the Mesh30's indices.
     * 
     * @param indices
     *            the array to copy the indices to
     * @param destOffset
    ...
    /**
     * Copies the remaining indices from the Mesh30 to the short array. The
     * short array must be large enough to hold destOffset + all the remaining
     * indices.
     * 
     * @param srcOffset
    ...
    /**
     * Copies the indices from the Mesh30 to the short array. The short array
     * must be large enough to hold destOffset + count indices.
     * 
     * @param srcOffset
     *            the zero-based offset of the first index to fetch
     * @param count
    ...
    /**
     * Sets whether to bind the underlying {@link VertexArray30} or
     ...
     * up memory if you manipulate the vertices and indices of a Mesh30 multiple
     ...
     */
    public void bind(final ShaderProgram30 shader) {
        bind(shader, null);
    }
     ...
     */
    public void bind(final ShaderProgram30 shader, final int[] locations) {
     ...
     */
    public void unbind(final ShaderProgram30 shader) {
        unbind(shader, null);
    }
     ...
     *            array containing the attribute locations.
     */
    public void unbind(final ShaderProgram30 shader, final int[] locations) {
     ...
     * 
     * <p>
     * This method must only be called after the {@link ShaderProgram30#begin()}
     ...
     *            the primitive type
     */
    public void render(ShaderProgram30 shader, int primitiveType) {
     ...
     */
    public void render(ShaderProgram30 shader, int primitiveType, int offset, int count) {
     ...
     *            number of vertices or indices to use
     * @param autoBind
     *            overrides the autoBind member of this Mesh30
     */
    public void render(ShaderProgram30 shader, int primitiveType, int offset, int count, boolean autoBind) {
        if (count == 0)
            return;
    ...
    /** Frees all resources associated with this Mesh30 */
    public void dispose() {
    ...
    /** @return the vertex attributes of this Mesh30 */
    ...
    private static void addManagedMesh(Application app, Mesh30 mesh) {
        Array<Mesh30> managedResources = meshes.get(app);
        if (managedResources == null)
            managedResources = new Array<Mesh30>();
    ...
    public static void invalidateAllMeshes(Application app) {
        Array<Mesh30> meshesArray = meshes.get(app);
        if (meshesArray == null)
            return;
     ...
     */
    public Mesh30 copy(boolean isStatic, boolean removeDuplicates, final int[] usage) {
        ...
        Mesh30 result;
        if (attrs == null)
            result = new Mesh30(isStatic, numVertices, indices == null ? 0 : indices.length, getVertexAttributes());
        else
            result = new Mesh30(isStatic, numVertices, indices == null ? 0 : indices.length, attrs);
     ...
     * @return the copy of this mesh
     */
    public Mesh30 copy(boolean isStatic) {
